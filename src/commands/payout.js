    ...
    try {
        ...
        if (!interaction.replied && !interaction.deferred) {
            await interaction.reply({
                embeds: [errorEmbed('An unexpected error occurred while processing payroll.')],
                flags: MessageFlags.Ephemeral,
            });
        }
    }
...
export const buttons = {
    payoutreceipt: async (interaction) => {
        try {
            ...
            console.error('[payout button]', err);
            if (!interaction.replied && !interaction.deferred) {
                await interaction.reply({
                    embeds: [errorEmbed('An unexpected error occurred while loading the payroll receipt.')],
                    flags: MessageFlags.Ephemeral,
                });
            }
        }
